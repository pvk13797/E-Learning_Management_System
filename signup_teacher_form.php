			<form id="signin_teacher" class="form-signin" method="post">
					...
					<button id="signin" name="login" class="btn btn-info" type="submit"><i class="icon-signin icon-large"></i> Sign in</button>
					<button id="clear" name="clear" class="btn btn-danger" type="reset"><i class="icon-refresh icon-large"></i> Clear</button>
			</form>
			<script>
			...
			jQuery("#signin_teacher").submit(function(e){
					...
					$.ajax({
						...
						{
						$.jGrowl("Password Not Match", { sticky: true });
						$.jGrowl("Your data is not found in the database", { header: 'Sign Up Failed' });
						}
				});
			});
